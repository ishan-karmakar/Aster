  ...
  async scheduled(
    ...
    ctx.waitUntil(
      Promise.all([
        ...
        rescheduleMissedWork(env),
        pullQueuedCalendarChanges(env).then(() => syncConnectedCalendars(env)),
      ]),
    );
  },
};
...
async function sendDueReminders(env: Env) {
  ...
  const due = await env.DB.prepare(
    "SELECT id,user_email,title,subject,due_at FROM assignments WHERE reminder_status='pending' AND reminder_at<=? AND reminder_attempts<5 ORDER BY reminder_at LIMIT 50",
  )
    .bind(now)
    .all<ReminderRow>();
  for (const item of due.results) {
    const home =
      env.HOME_URL || "https://aster-homework-planner.yashman9012.chatgpt.site";
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      headers: {
        ...
        "User-Agent": "Aster-Homework-Planner/1.0",
        "Idempotency-Key": `assignment-reminder/${item.id}`,
      },
      body: JSON.stringify({
        ...
        subject,
        html,
        text: `${item.title} for ${item.subject} is due ${new Date(item.due_at).toLocaleString()}. Open Aster: ${home}`,
      }),
    });
    if (response.ok)
      await env.DB.prepare(
        "UPDATE assignments SET reminder_status='sent' WHERE id=?",
      )
        .bind(item.id)
        .run();
    else
      await env.DB.prepare(
        "UPDATE assignments SET reminder_attempts=reminder_attempts+1 WHERE id=?",
      )
        .bind(item.id)
        .run();
  }
...
async function rescheduleMissedWork(env: Env) {
  await env.DB.prepare(
    "CREATE TABLE IF NOT EXISTS planner_state (user_email TEXT PRIMARY KEY NOT NULL, state TEXT NOT NULL, updated_at INTEGER NOT NULL DEFAULT (unixepoch()))",
  ).run();
  const rows = await env.DB.prepare(
    "SELECT user_email,state FROM planner_state LIMIT 100",
  ).all<{ user_email: string; state: string }>();
  for (const row of rows.results) {
    try {
      const before = normalizePlannerState(
          JSON.parse(row.state) as PlannerState,
        ),
        next = markMissedAndReschedule(before);
      if (next.revisions.length !== before.revisions.length) {
        next.revisions[0].explanation = await explainScheduleChange(
          env.OPENAI_API_KEY,
          next.revisions[0].reason,
          next.revisions[0].explanation,
        );
        await env.DB.prepare(
          "UPDATE planner_state SET state=?,updated_at=unixepoch() WHERE user_email=?",
        )
          .bind(JSON.stringify(next), row.user_email)
          .run();
      }
    } catch {
      /* Keep processing other students if one record is malformed. */
    }
  }
}
...
async function syncConnectedCalendars(env: Env) {
  ...
  const connections = await env.DB.prepare(
    "SELECT user_email,refresh_token,calendar_id FROM calendar_connections WHERE provider='google' AND status IN ('connected','error') ORDER BY updated_at LIMIT 20",
  ).all<{ user_email: string; refresh_token: string; calendar_id: string }>();
  for (const connection of connections.results) {
    try {
      const refresh = await decryptWorkerToken(
      ...
      const row = await env.DB.prepare(
        "SELECT state FROM planner_state WHERE user_email=?",
      )
      ...
      const state = normalizePlannerState(
        ...
        links = await env.DB.prepare(
          "SELECT session_id,event_id,session_updated FROM calendar_event_links WHERE user_email=?",
        )
          .bind(connection.user_email)
          .all<{
            session_id: string;
        ...
        bySession = new Map(
          links.results.map((item) => [item.session_id, item]),
        ),
        calendarId = encodeURIComponent(connection.calendar_id || "primary");
      for (const session of state.sessions) {
        const link = bySession.get(session.id);
        if (session.status !== "planned") {
          if (link) {
            await fetch(
              `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${encodeURIComponent(link.event_id)}`,
              {
                method: "DELETE",
            ...
            await env.DB.prepare(
              ...
              .bind(connection.user_email, session.id)
              .run();
          }
          continue;
        }
        const event = {
          ...
          description: "Aster study session",
          start: {
        ...
        if (!link) {
          const response = await fetch(
            ...
            created = (await response.json()) as {
              id?: string;
              updated?: string;
            };
          if (created.id)
            await env.DB.prepare(
              ...
              .run();
        } else if (session.updatedAt > (link.session_updated || ""))
          await fetch(
            `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${encodeURIComponent(link.event_id)}`,
            {
              method: "PATCH",
      ...
      await env.DB.prepare(
        "UPDATE calendar_connections SET status='connected',updated_at=unixepoch() WHERE user_email=?",
      )
        .bind(connection.user_email)
        .run();
    } catch {
...
async function pullQueuedCalendarChanges(env: Env) {
  if (
    ...
    return;
  await env.DB.prepare(
    "CREATE TABLE IF NOT EXISTS calendar_sync_queue (user_email TEXT PRIMARY KEY NOT NULL, requested_at INTEGER NOT NULL DEFAULT (unixepoch()),attempts INTEGER NOT NULL DEFAULT 0,next_attempt_at INTEGER NOT NULL DEFAULT 0,last_error TEXT)",
  ).run();
  const queued = await env.DB.prepare(
    "SELECT c.user_email,c.refresh_token,c.calendar_id FROM calendar_connections c JOIN calendar_sync_queue q ON q.user_email=c.user_email WHERE c.provider='google' AND q.next_attempt_at<=unixepoch() AND q.attempts<8 ORDER BY q.requested_at LIMIT 20",
  ...
  for (const connection of queued.results) {
    try {
      ...
      const state = normalizePlannerState(
        ...
        links = await env.DB.prepare(
          "SELECT session_id,event_id,event_updated,session_updated FROM calendar_event_links WHERE user_email=?",
        )
          .bind(connection.user_email)
          .all<{
            session_id: string;
        ...
        eventsData = (await eventsResponse.json()) as {
          items?: WorkerGoogleEvent[];
        },
        events = new Map(
          (eventsData.items || []).map((item) => [item.id, item]),
        );
      for (const link of links.results) {
        const session = state.sessions.find(
            (item) => item.id === link.session_id,
          ),
        ...
        if (!event || event.status === "cancelled") {
          if ((event?.updated || "") >= (session.updatedAt || "")) {
            ...
            session.updatedAt = event?.updated || new Date().toISOString();
          }
          continue;
        }
        const googleChanged =
            (event.updated || "") > (link.event_updated || ""),
          asterChanged = session.updatedAt > (link.session_updated || "");
        if (
          googleChanged &&
          ...
          session.start = event.start.dateTime;
          session.end = event.end.dateTime;
          session.updatedAt = event.updated || new Date().toISOString();
        }
      }
      const busyResponse = await fetch(
        ...
        busyData = (await busyResponse.json()) as {
          items?: WorkerGoogleEvent[];
        },
      ...
      await env.DB.batch([
        env.DB.prepare(
          "UPDATE planner_state SET state=?,updated_at=unixepoch() WHERE user_email=?",
        ).bind(JSON.stringify(state), connection.user_email),
        env.DB.prepare(
          "DELETE FROM calendar_sync_queue WHERE user_email=?",
        ).bind(connection.user_email),
      ]);
    } catch (error) {
      const message =
      ...
      await env.DB.batch([
        ...
        env.DB.prepare(
          "UPDATE calendar_sync_queue SET attempts=attempts+1,next_attempt_at=unixepoch()+MIN(3600,60*(1 << attempts)),last_error=? WHERE user_email=?",
        ).bind(message, connection.user_email),
      ]);
    }
  }
}
async function decryptWorkerToken(value: string, secret: string) {
  const encoder = new TextEncoder(),
    [iv, payload] = value.split("."),
    decode = (input: string) =>
      Uint8Array.from(atob(input), (character) => character.charCodeAt(0)),
    digest = await crypto.subtle.digest("SHA-256", encoder.encode(secret)),
    key = await crypto.subtle.importKey("raw", digest, "AES-GCM", false, [
      "decrypt",
    ]),
...
function reminderEmail(item: ReminderRow, home: string) {
  const title = escapeHtml(item.title),
    course = escapeHtml(item.subject),
    url = escapeHtml(home),
    due = escapeHtml(
